...
class ASPP1(nn.Module):
    def __init__(self, in_dims, out_dims, rate=[1,6, 12, 18]):
        ...
        self.dilate2 = nn.Conv2d(512, 512, kernel_size=3, dilation=3, padding=3)
        self.dilate3 = nn.Conv2d(512, 512, kernel_size=3, dilation=5, padding=5)
        self.conv1x1 = nn.Conv2d(512, 512, kernel_size=1, dilation=1, padding=0)
    ...
    def forward(self, x):
        ...
        x2 = self.aspp_block2(self.dilate1(x))
        x3 = self.aspp_block3(self.dilate2(self.dilate1(x)))
        x4 = self.aspp_block4(self.dilate3(self.dilate2(self.dilate1(x))))
...
class SPPblock(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        self.layer1 = self.conv(self.pool1(x))
        self.layer2 = self.conv(self.pool2(x))
        self.layer3 = self.conv(self.pool3(x))
        self.layer4 = self.conv(self.pool4(x))
        '''
...
class CE_Net_(nn.Module):
    def __init__(self, num_classes=1, num_channels=3):
        ...
        self.Up4 = up_conv(ch_in=516, ch_out=256) #换成cenet这里要把512-516
        ...
        self.Conv_1x1 = nn.Conv2d(64, 64, kernel_size=1, stride=1, padding=0)

        self.decoder4 = DecoderBlock(512, filters[2])   #只有加SE模块 这里改成516-512
    ...
    def forward(self, x):
        ...
        # Decoder加注意力机制
        '''
        #e4 = self.Att5(g=e4, x=e3)
        d4 = self.Up4(e4)
        #d4 = self.decoder4(e4)
        d4 = self.Att4(g=d4, x=e3)
        d4 = torch.cat((e3, d4), dim=1)
        d4 = self.Up_conv4(d4)

        d3 = self.Up3(d4)
        #d3 = self.decoder3(d4)
        d3 = self.Att3(g=d3, x=e2)
        d3 = torch.cat((e2, d3), dim=1)
        d3 = self.Up_conv3(d3)

        d2 = self.Up2(d3)
        #d2 = self.decoder2(d3)
        d2 = self.Att2(g=d2, x=e1)
